...
- **WATCH** — passive. PR events arrive via mailbox delivery (see the
  ...
  - comment/CHANGES_REQUESTED event → the event body carries the full text of
    ...
    Approved → `respond "Address review comments: <threads/summary>" --thread --worktree <slug> --slug respond-pr-<PR#>`,
...
1. Rounds are counted per loop: CI-fix rounds and comment-response rounds are
   separate caps of 3. Slug stems track them automatically (`fix` → `fix-r2`,
   `respond-pr-3` → `respond-pr-3-r2`) — respond stems include the PR number
   since they're always about one.
