...
func init() {
	mailboxInboxCmd.Flags().BoolVar(&mboxUnread, "unread", false, "show only unread messages")
	mailboxInboxCmd.Flags().BoolVar(&mboxFollow, "follow", false, "keep watching for new messages")
	mailboxSendCmd.Flags().StringVar(&mboxStatus, "status", "", "report task status: "+joinTaskStatuses())
	mailboxCmd.AddCommand(mailboxInboxCmd, mailboxSendCmd)
...
var mailboxSendCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if mboxStatus != "" && !store.ValidTaskStatus(mboxStatus) {
			return fmt.Errorf("invalid status %q; valid: %s", mboxStatus, joinTaskStatuses())
		}
		from := store.SenderLabel(t.PaneID)
		m := &store.Message{TaskID: t.ID, From: from, Body: args[1], Status: mboxStatus}
		if flagDryRun {
