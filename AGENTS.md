...
task **type** as `from` (`plan`, `build`, `test`, ...); foreman messages use
`from: foreman`; watch events use `from: watch`. Messages never carry tab
IDs — tabs close when tasks finish, so only durable identifiers (task ID,
